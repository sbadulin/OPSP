...
void countWords(char arr[]) {
	...
	printf("\nWord count: %d\n", ret);
	return; // Возвращаем значение счетчика слов
}
...
void сountFileWords(FILE* file) {
	...
	if (file != NULL) { // отлавливаем проблемы с файлом
		...
		fscanf(file, "%c", &ret); // убираем лишний символ из потока ввода
	}
	else { // если есть проблемы с файлом, то выбрасываем предупреждение
		printf("file troubles\n");
	}
	
	// считаем количество слов с помощью Задачи 3
